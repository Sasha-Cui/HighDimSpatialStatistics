...
& \(h,\nu\), boundary, irregularity, domain size
& Model containment, Monte Carlo error to target, retained boundary fits \\
...
order 96. Appendix~\ref{app:dimension-kernel-audit} audits the same phase
coefficient in dimensions one through three and for a second compact kernel.
...
This differs from classical range-of-influence rules: compactly supported
variograms can gain a geometric support-width extension
\cite{clark1977regularization}, but Matérn has no finite range and our
variance-refitted KL target moves at a smoothness-dependent power.
Rough
...
tag \texttt{supportshift-geosim-v1.2.1}. The release includes a synthetic-data
...
finite-grid fits, 12,800 replicated fits, and 113 reported numerical claims.
...
The primary phase figure fixes \(d=2\) and the product Epanechnikov kernel.
As a separate, predeclared implementation audit, we used
...
order 48 per coordinate and was repeated at orders 32 and 64. These factors
give 72 deterministic cells.
...
\[
 ...
 \end{cases}
\]
and let \(C_{\nu,k}\) be the positive coefficient of
\(\alpha-\alpha_h^\dagger\) in Theorem~\ref{thm:phase-law}. Table~\ref{tab:dimension-kernel}
reports the range, over the four smoothness values, of
\((\alpha-\alpha_h^\dagger)/\{C_{\nu,k}s_\nu(h)\}\) at \(h=0.002\).

\begin{table}[h]
\centering
\small
\caption{Smallest-bandwidth coefficient ratios in the robustness audit.}
\label{tab:dimension-kernel}
\input{tables/dimension_kernel_robustness.tex}
\end{table}

All 72 exact shifts were positive. The largest absolute coefficient-ratio
error was 0.169, below the predeclared 0.20 gate; it occurred at \(\nu=1\),
where logarithmic convergence is slow. The maximum relative shift change under
quadrature refinement was \(3.92\times10^{-7}\), below the predeclared
\(2\times10^{-4}\) gate. This finite audit checks two kernel families and
three dimensions only; it is neither the proof of the kernel-general theorem
nor a uniform guarantee over kernels or dimensions.
