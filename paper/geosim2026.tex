...
tag \texttt{supportshift-geosim-v1.2.0}. The release includes a synthetic-data
card, scoped reuse terms, a citation file, and a one-command verifier for the
promoted inputs, all paper artifacts, and 113 reported numerical claims.
...
Let \(f_\alpha(x)=\Matern_\nu(\alpha\lVert x\rVert)\).  Its first four
...
\begin{equation}
 \E f_\alpha(r+hD)
 =f_\alpha(r)+\frac{h^2}{2}
  \tr\{\nabla^2f_\alpha(r)\E(DD^\top)\}+O(h^4).
 \label{eq:appendix-taylor}
\end{equation}
For \(r=Re\), the radial Hessian is
\[
 \nabla^2f_\alpha(r)=\alpha^2\Matern_\nu''(z)ee^\top
...
\begin{align}
 ...
 \Matern_2(x)
 &=1-\frac{x^2}{4}
   +x^4\left\{\frac3{64}-\frac{\log(x/2)+\gamma_{\rm E}}{16}\right\}
   +O\{x^6(1+|\log x|)\}.
\end{align}
