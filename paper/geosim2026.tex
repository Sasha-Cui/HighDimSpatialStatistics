...
\title{How Local Averaging Moves Matérn Pairwise Range Targets:
...
\begin{abstract}
Spatial measurements often represent local averages but are fitted as point
observations. This changes the population target of covariance estimation, not
merely its finite-sample bias. We study a stationary Matérn Gaussian field in
\(\R^d\), observed through a known compact symmetric averaging kernel of
bandwidth \(h\). For a fixed nonzero-lag Gaussian two-point likelihood with
fixed known smoothness \(\nu\), we characterize the exact variance and decay
targets. The naive
decay shift is of order \(h^{2\nu}\) for \(0<\nu<1\),
\(h^2\log(1/h)\) at \(\nu=1\), and \(h^2\) for \(\nu>1\). Explicit
coefficients show that naive point-support fitting underestimates inverse
range---and thus overestimates spatial range---for every \(\nu>0\) and any
nondegenerate compact symmetric kernel when \(h\) is sufficiently small. The
smooth-regime sign remains valid for anisotropic averaging. Deterministic
two-dimensional quadrature verifies the three rate regimes. A 200-replicate
finite-grid study then compares naive and support-aware full likelihoods,
including boundary and irregular-design stress tests. The result isolates an
inferential failure mode that geospatial simulation studies can reproduce and
diagnose without an empirical dataset.
\end{abstract}
...
misspecified likelihood, spatial simulation, range estimation}
...
\cite{gelfand2001support,chacon2024change}. A common shortcut instead assigns
each aggregate to a declared center and fits a point-support covariance.
Relative to the latent parameters, the support is then misinterpreted. A single
two-point covariance can still be matched exactly by different variance and
decay parameters; a multi-lag likelihood is genuinely misspecified and selects
a Kullback--Leibler (KL) projection \cite{bachoc2018misspecified}.

This qualitative fact leaves three quantitative questions. How quickly does the
pseudo-parameter move as support shrinks? Does the rate depend on Matérn
smoothness? Is the direction of range error stable across dimensions and
averaging kernels? The answer is not an ordinary second-order Taylor expansion:
the Matérn covariance changes regularity at the origin as smoothness crosses
one.

Temporal aggregation of autoregressive and Ornstein--Uhlenbeck processes is
classical \cite{amemiya1972aggregation,stram1986aggregation,folia2018aggregated}.
Change-of-support reviews and area-to-point methods establish block covariance
and prediction \cite{gotway2002incompatible,kyriakidis2005areal}. Most closely,
\cite{fuentes2007approximate} builds a likelihood for block averages from an
underlying point covariance and includes a Matérn example, but does not derive
the parameter selected when those averages are instead treated as points.
Generic misspecified-GP theory gives the KL framework but not this coefficient
or sign \cite{bachoc2018misspecified}. We make no novelty claim for a
one-dimensional exponential calculation or for \(SKS^\top\). Our contribution
is an all-smoothness, \(d\)-dimensional fixed-lag expansion of the naive Matérn
pair-likelihood target:

\begin{itemize}
  \item The leading inverse-range shift has order \(h^{2\nu}\),
  \(h^2\log(1/h)\), or \(h^2\), depending on whether \(\nu<1\),
  \(\nu=1\), or \(\nu>1\).
  \item Explicit coefficients and a Bessel-function identity prove that the
  shift is negative for every \(\nu>0\), including under anisotropic symmetric
  averaging.
  \item A reproducible geospatial simulation protocol validates the continuous
  phase law and compares naive and support-aware full-grid likelihoods.
\end{itemize}

We use increasing-domain language only for repeated pair criteria. We do not
claim separate fixed-domain consistency of Matérn variance and range, which
would conflict with microergodicity results \cite{zhang2004inconsistent}.
